...
somfy_commands translateSomfyCommand(const String &string)
{
  if (string.equalsIgnoreCase("My"))
    ...
    return somfy_commands::UpDown;
  else if (string.equalsIgnoreCase("MyUpDown"))
    return somfy_commands::MyUpDown;
  else if (string.equalsIgnoreCase("Prog"))
    return somfy_commands::Prog;
  else if (string.equalsIgnoreCase("SunFlag"))
    return somfy_commands::SunFlag;
  else if (string.equalsIgnoreCase("StepUp"))
    return somfy_commands::StepUp;
  else if (string.equalsIgnoreCase("StepDown"))
    return somfy_commands::StepDown;
  else if (string.equalsIgnoreCase("Flag"))
    return somfy_commands::Flag;
  else if (string.equalsIgnoreCase("Sensor"))
    return somfy_commands::Sensor;
  else if (string.equalsIgnoreCase("Toggle"))
    return somfy_commands::Toggle;
  else if (string.equalsIgnoreCase("Favorite"))
    return somfy_commands::Favorite;
  else if (string.equalsIgnoreCase("Stop"))
    return somfy_commands::Stop;
  else if (string.startsWith("fav") || string.startsWith("FAV"))
    return somfy_commands::Favorite;
  else if (string.startsWith("mud") || string.startsWith("MUD"))
    return somfy_commands::MyUpDown;
  else if (string.startsWith("md") || string.startsWith("MD"))
    return somfy_commands::MyDown;
  else if (string.startsWith("ud") || string.startsWith("UD"))
    return somfy_commands::UpDown;
  else if (string.startsWith("mu") || string.startsWith("MU"))
    return somfy_commands::MyUp;
  else if (string.startsWith("su") || string.startsWith("SU"))
    return somfy_commands::StepUp;
  else if (string.startsWith("sd") || string.startsWith("SD"))
    return somfy_commands::StepDown;
  else if (string.startsWith("sen") || string.startsWith("SEN"))
    return somfy_commands::Sensor;
  else if (string.startsWith("p") || string.startsWith("P"))
    return somfy_commands::Prog;
  else if (string.startsWith("u") || string.startsWith("U"))
    return somfy_commands::Up;
  else if (string.startsWith("d") || string.startsWith("D"))
    return somfy_commands::Down;
    ...
    return somfy_commands::SunFlag;
  else if (string.startsWith("t") || string.startsWith("T"))
    return somfy_commands::Toggle;
...
void somfy_frame_t::decodeFrame(byte *frame)
{
  ...
  if (this->cmd == somfy_commands::RTWProto)
  {
    if (this->encKey >= 160)
    {
      ...
      if (this->encKey == 164)
        this->cmd = somfy_commands::Toggle;
    }
...
void somfy_frame_t::encode80BitFrame(byte *frame, uint8_t repeat)
{
  ...
  case somfy_commands::StepUp:
    if (repeat == 0)
      frame[1] = (static_cast<byte>(somfy_commands::StepDown) << 4) | (frame[1] & 0x0F);
    if (this->stepSize == 0)
      this->stepSize = 1;
  ...
  case somfy_commands::Favorite:
    if (repeat == 0)
      frame[1] = (static_cast<byte>(somfy_commands::My) << 4) | (frame[1] & 0x0F);
...
void somfy_frame_t::encodeFrame(byte *frame)
{
  ...
  else
  {
    if (this->bitLength == 80)
      this->encode80BitFrame(&frame[0], this->repeats);
  }
  byte checksum = 0;
...
SomfyShade *SomfyShadeController::findShadeByRemoteAddress(uint32_t address)
{
  for (uint8_t i = 0; i < SOMFY_MAX_SHADES; i++)
  {
    ...
    else
    {
      ...
      {
        if (shade.linkedRemotes[j].getRemoteAddress() == address)
          return &shade;
      }
    }
  }
...
SomfyGroup *SomfyShadeController::findGroupByRemoteAddress(uint32_t address)
{
  for (uint8_t i = 0; i < SOMFY_MAX_GROUPS; i++)
  {
    ...
    if (group.getRemoteAddress() == address)
      return &group;
  }
  return nullptr;
}
...
#ifdef USE_NVS
  if (!this->useNVS())
  { // At 1.4 we started using the configuration file.  If the file doesn't exist then booh.
      ...
      if (somfy.useNVS())
      {
        ...
        for (uint8_t i = 0; i < SOMFY_MAX_LINKED_REMOTES; i++)
        {
          ...
          if (lremote.getRemoteAddress() != 0)
            linkedAddresses[j++] = lremote.getRemoteAddress();
        }
        char shadeKey[15];
...
bool SomfyShade::isAtTarget()
{
  ...
  if (this->tiltType == tilt_types::tiltonly)
    return fabs(this->currentTiltPos - this->tiltTarget) < epsilon;
  else if (this->tiltType == tilt_types::none)
    return fabs(this->currentPos - this->target) < epsilon;
...
{
  if (this->getShadeId() == 255)
    return false;
  for (uint8_t i = 0; i < SOMFY_MAX_GROUPS; i++)
  {
    if (somfy.groups[i].getGroupId() != 255 && somfy.groups[i].hasShadeId(this->getShadeId()))
      return true;
  }
  return false;
}
  ...
  if (this->proto == radio_proto::GP_Relay)
  {
    ...
    else
    {
      ...
      case -1:
        ...
        digitalWrite(this->gpioUp, p_on);
        if (dir != this->gpioDir)
          Serial.printf("UP: true, DOWN: false\n");
        this->gpioDir = dir;
        break;
      case 1:
        digitalWrite(this->gpioUp, p_off);
        digitalWrite(this->gpioDown, p_on);
        if (dir != this->gpioDir)
          Serial.printf("UP: false, DOWN: true\n");
        this->gpioDir = dir;
        break;
      default:
        digitalWrite(this->gpioUp, p_off);
        digitalWrite(this->gpioDown, p_off);
        if (dir != this->gpioDir)
          Serial.printf("UP: false, DOWN: false\n");
        this->gpioDir = dir;
        break;
      }
    }
  ...
  if (this->proto == radio_proto::GP_Remote)
  {
    ...
    case somfy_commands::My:
      if (this->shadeType != shade_types::drycontact && !this->isToggle())
      {
        ...
        dir = 0;
        if (dir != this->gpioDir)
          Serial.printf("UP: false, DOWN: false, MY: true\n");
      }
      break;
    case somfy_commands::Up:
...
void SomfyShade::computeAlphas()
{
  ...
  if (this->downMidTime > 0 && this->downMidTime < this->downTime)
  {
    ...
    {
      float f = log(2.0f) + (1.0f - r) * log(a) - log(a + 1.0f);
      float fp = (1.0f - r) / a - 1.0f / (a + 1.0f);
      a -= f / fp;
      if (a <= 1.001f)
      {
        a = 1.001f;
  ...
  if (this->upMidTime > 0 && this->upMidTime < this->upTime)
  {
    ...
    for (uint8_t i = 0; i < 10; i++)
    {
      float f = log(a + 1.0f) - log(2.0f) - r * log(a);
      float fp = 1.0f / (a + 1.0f) - r / a;
      a -= f / fp;
      if (a <= 1.001f)
      {
        a = 1.001f;
...
void SomfyShade::checkMovement()
{
  ...
  this->p_direction(this->currentPos == this->target ? 0 : this->currentPos > this->target ? -1
                                                                                           : 1);
  ...
  this->p_tiltDirection(this->currentTiltPos == this->tiltTarget ? 0 : this->currentTiltPos > this->tiltTarget ? -1
                                                                                                               : 1);
  if (tilt_first)
  ...
  {
    if (isSunny && !isWindy)
    { // It is sunny and there is no wind so we should be extended
      if (this->noWindDone && !this->sunDone && this->sunStart && (curTime - this->sunStart) >= SOMFY_SUN_TIMEOUT)
      {
      ...
      if (!this->noWindDone && this->noWindStart && (curTime - this->noWindStart) >= SOMFY_NO_WIND_TIMEOUT)
      {
        this->p_target(this->myPos >= 0 ? this->myPos : 100.0f);
        // this->target = this->myPos >= 0 ? this->myPos : 100.0f;
        this->noWindDone = true;
    ...
    if (!isSunny && !this->noSunDone && this->noSunStart && (curTime - this->noSunStart) >= SOMFY_NO_SUN_TIMEOUT)
    {
      if (this->tiltType == tilt_types::tiltonly)
  ...
  if (isWindy && !this->windDone && this->windStart && (curTime - this->windStart) >= SOMFY_WIND_TIMEOUT)
  {
    if (this->tiltType == tilt_types::tiltonly)
      this->p_tiltTarget(0.0f);
    this->p_target(0.0f);
    ...
    else if (this->currentTiltPos >= this->tiltTarget)
    {
      ...
      if (this->settingTiltPos)
      {
        if (this->tiltType == tilt_types::integrated)
        {
          ...
          if (this->tiltTarget != 100.0f || this->currentPos != 100.0f)
            SomfyRemote::sendCommand(somfy_commands::My, this->repeats);
        }
        else
        {
          // This is a tilt motor so let it complete if it is going to 100.
          if (this->tiltTarget != 100.0f)
            SomfyRemote::sendCommand(somfy_commands::My, this->repeats);
        }
      }
      this->p_tiltDirection(0);
    ...
    else if (this->currentTiltPos <= this->tiltTarget)
    {
      ...
      if (this->settingTiltPos)
      {
        if (this->tiltType == tilt_types::integrated)
        {
          // If this is an integrated tilt mechanism the we will simply let it finish.  If it is not then we will stop it.
          // Serial.printf("Sending My -- tiltTarget: %.2f, tiltDirection: %d\n", this->tiltTarget, this->tiltDirection);
          if (this->tiltTarget != 0.0 || this->currentPos != 0.0)
            SomfyRemote::sendCommand(somfy_commands::My, this->repeats);
        }
        else
        {
          // This is a tilt motor so let it complete if it is going to 0.
          if (this->tiltTarget != 0.0)
            SomfyRemote::sendCommand(somfy_commands::My, this->repeats);
        }
      }
      this->p_tiltDirection(0);
  ...
  if (this->settingMyPos && this->isAtTarget())
  {
    ...
    {
      if (this->myTiltPos == this->currentTiltPos && this->myPos == this->currentPos)
...
{
  if (!mqtt.connected() || !settings.MQTT.pubDisco)
    return;
  char topic[128] = "";
...
float SomfyShade::p_currentPos(float pos)
{
  ...
  this->currentPos = pos;
  if (floor(old) != floor(pos))
    this->publish("position", this->transformPosition(static_cast<uint8_t>(floor(this->currentPos))));
  return old;
}
float SomfyShade::p_currentTiltPos(float pos)
{
  ...
  if (floor(old) != floor(pos))
    this->publish("tiltPosition", this->transformPosition(static_cast<uint8_t>(floor(this->currentTiltPos))));
  return old;
}
uint16_t SomfyShade::p_lastRollingCode(uint16_t code)
{
  ...
  if (old != code)
    this->publish("lastRollingCode", code);
  return old;
}
...
bool SomfyShade::p_sunFlag(bool val)
{
  ...
  if (old != val)
    this->publish("sunFlag", static_cast<uint8_t>(val));
  return old;
}
bool SomfyShade::p_windy(bool val)
{
  ...
  if (old != val)
    this->publish("windy", static_cast<uint8_t>(val));
  return old;
}
bool SomfyShade::p_sunny(bool val)
{
  ...
  if (old != val)
    this->publish("sunny", static_cast<uint8_t>(val));
  return old;
}
...
void SomfyShade::processWaitingFrame()
{
  ...
  if (this->lastFrame.await > 0 && (millis() > this->lastFrame.await))
  {
    ...
    case somfy_commands::Up:
      ...
      { // Theoretically this should get here unless it does have a tilt motor.
    ...
    case somfy_commands::My:
      ...
      else if (this->isIdle())
      {
        ...
        {
          if (this->myPos >= 0.0f && this->myPos <= 100.0f)
            this->p_target(this->myPos);
          if (this->myTiltPos >= 0.0f && this->myTiltPos <= 100.0f)
            this->p_tiltTarget(this->myTiltPos);
        }
        this->setMovement(0);
        this->lastFrame.processed = true;
        this->emitCommand(cmd, "remote", this->lastFrame.remoteAddress);
      ...
      if (this->lastFrame.repeats > SETMY_REPEATS + 2)
        this->lastFrame.processed = true;
      if (this->lastFrame.processed)
      {
        Serial.print(this->name);
...
void SomfyShade::processFrame(somfy_frame_t &frame, bool internal)
{
  ...
  if (!hasRemote)
  {
    for (uint8_t i = 0; i < SOMFY_MAX_LINKED_REMOTES; i++)
    {
      ...
      {
        if (frame.cmd != somfy_commands::Sensor)
  ...
  case somfy_commands::Flag:
    ...
    if (this->shadeType == shade_types::drycontact || this->shadeType == shade_types::drycontact2)
      return;
    if (this->lastFrame.rollingCode & 0x8000)
      return; // Some sensors send bogus frames with a rollingCode >= 32768 that cause them to change the state.
  ...
  case somfy_commands::Up:
    ...
    else
    {
      ...
      if (this->tiltType == tilt_types::tiltonly && !internal)
        this->p_tiltTarget(0.0f);
      else if (!internal)
      {
        if (this->tiltType != tilt_types::tiltonly)
  ...
  case somfy_commands::My:
    ...
    { // This is a one button device
      ...
      if (!this->isIdle())
        ...
        this->p_target(this->lastMovement == -1 ? 100 : 0);
      this->emitCommand(cmd, internal ? "internal" : "remote", frame.remoteAddress);
      return;
    }
    ...
    if (this->isIdle())
    {
      ...
      else
      {
        ...
        if (this->myTiltPos >= 0.0f && this->myTiltPos <= 100.0f)
          this->p_tiltTarget(this->myTiltPos);
        if (this->myPos >= 0.0f && this->myPos <= 100.0f && this->tiltType != tilt_types::tiltonly)
          this->p_target(this->myPos);
        this->emitCommand(cmd, internal ? "internal" : "remote", frame.remoteAddress);
      }
    }
  ...
  case somfy_commands::StepUp:
    ...
    if (this->lastFrame.stepSize == 0)
      this->lastFrame.stepSize = 1;
    if (this->tiltType == tilt_types::integrated)
    {
      ...
      if (this->currentTiltPos <= 0.0f && this->currentPos <= 0.0f)
        return; // Do nothing
    ...
    {
      if (this->tiltTime == 0 || this->stepSize == 0)
        return;
      this->p_tiltTarget(max(0.0f, this->currentTiltPos - (100.0f / (static_cast<float>(this->tiltTime / static_cast<float>(this->stepSize * this->lastFrame.stepSize))))));
    }
    else if (this->currentPos > 0.0f)
    {
      if (this->downTime == 0 || this->stepSize == 0)
        return;
      this->p_target(max(0.0f, this->currentPos - (100.0f / (static_cast<float>(this->upTime / static_cast<float>(this->stepSize * this->lastFrame.stepSize))))));
    }
    this->emitCommand(cmd, internal ? "internal" : "remote", frame.remoteAddress);
  ...
  case somfy_commands::StepDown:
    ...
    if (this->tiltType == tilt_types::integrated)
    {
      ...
      if (this->currentTiltPos >= 100.0f && this->currentPos >= 100.0f)
        return; // Do nothing
...
void SomfyShade::processInternalCommand(somfy_commands cmd, uint8_t repeat)
{
  ...
  case somfy_commands::StepUp:
    ...
    {
      if (this->tiltTime == 0 || this->currentTiltPos <= 0.0f)
        return;
      this->p_tiltTarget(max(0.0f, this->currentTiltPos - (100.0f / (static_cast<float>(this->tiltTime / static_cast<float>(this->stepSize))))));
    }
    else if (this->currentPos > 0.0f)
  ...
  case somfy_commands::StepDown:
    ...
    {
      if (this->tiltTime == 0 || this->stepSize == 0 || this->currentTiltPos >= 100.0f)
        return;
      this->p_tiltTarget(min(100.0f, this->currentTiltPos + (100.0f / (static_cast<float>(this->tiltTime / static_cast<float>(this->stepSize))))));
    }
    else if (this->currentPos < 100.0f)
...
void SomfyShade::setMovement(int8_t dir)
{
  ...
  if (dir == 0)
  {
    if (currDir != dir || currTiltDir != dir)
      this->commitShadePosition();
  }
  else
  {
...
{
  if (!this->isIdle())
    return; // Don't do this if it is moving.
  if (this->tiltType == tilt_types::tiltonly)
  {
    this->p_myPos(-1.0f);
...
void SomfyShade::moveToMyPosition()
{
  ...
  if (this->currentPos == this->myPos)
  {
    ...
    {
      if (this->currentTiltPos == this->myTiltPos)
        return; // Nothing to see here since we are already here.
    }
    else
      return;
  }
  if (this->myPos == -1 && (this->tiltType == tilt_types::none || this->myTiltPos == -1))
    return;
  if (this->tiltType != tilt_types::tiltonly && this->myPos >= 0.0f && this->myPos <= 100.0f)
    this->p_target(this->myPos);
  if (this->myTiltPos >= 0.0f && this->myTiltPos <= 100.0f)
    this->p_tiltTarget(this->myTiltPos);
  this->settingPos = false;
...
void SomfyShade::sendCommand(somfy_commands cmd, uint8_t repeat, uint8_t stepSize)
{
  ...
  {
    if (this->bitLength != 80)
      SomfyRemote::sendCommand(somfy_commands::My, repeat, stepSize);
    else
...
bool SomfyShade::usesPin(uint8_t pin)
{
  ...
  {
    if (this->proto == radio_proto::GP_Relay && this->gpioUp == pin)
      return true;
  }
  else if (this->shadeType == shade_types::drycontact2)
  {
    if (this->proto == radio_proto::GP_Relay && (this->gpioUp == pin || this->gpioDown == pin))
      return true;
  }
  else
  {
    if (this->gpioUp == pin)
      return true;
    else if (this->proto == radio_proto::GP_Remote && this->gpioMy == pin)
      return true;
  }
  return false;
}
  ...
  if (obj.containsKey("proto"))
  {
    ...
    if (proto == radio_proto::GP_Relay || proto == radio_proto::GP_Remote)
    {
      ...
      if (type == shade_types::drycontact ||
          ((type == shade_types::garage1 || type == shade_types::lgate1 || type == shade_types::cgate1 || type == shade_types::rgate1) && proto == radio_proto::GP_Remote))
        upPin = myPin = 255;
      else if (type == shade_types::drycontact2)
        myPin = 255;
      ...
      if (ret == 0)
      {
        for (uint8_t i = 0; i < SOMFY_MAX_SHADES; i++)
        {
          ...
          if (shade->getShadeId() == this->getShadeId() || shade->getShadeId() == 255)
            continue;
...
int8_t SomfyShade::fromJSON(JsonObject &obj)
{
  ...
  if (err == 0)
  {
    ...
    if (obj.containsKey("tiltTime"))
      this->tiltTime = obj["tiltTime"];
    if (obj.containsKey("stepSize"))
      this->stepSize = obj["stepSize"];
    ...
    if (obj.containsKey("bitLength"))
      this->bitLength = obj["bitLength"];
    if (obj.containsKey("proto"))
      this->proto = static_cast<radio_proto>(obj["proto"].as<uint8_t>());
    if (obj.containsKey("sunSensor"))
      this->setSunSensor(obj["sunSensor"]);
    if (obj.containsKey("simMy"))
      this->setSimMy(obj["simMy"]);
    if (obj.containsKey("light"))
      this->setLight(obj["light"]);
    if (obj.containsKey("gpioFlags"))
      this->gpioFlags = obj["gpioFlags"];
    ...
    if (obj.containsKey("flipCommands"))
      this->flipCommands = obj["flipCommands"].as<bool>();
    if (obj.containsKey("flipPosition"))
      this->flipPosition = obj["flipPosition"].as<bool>();
    ...
    if (this->proto == radio_proto::GP_Remote || this->proto == radio_proto::GP_Relay)
    {
      if (obj.containsKey("gpioUp"))
        this->gpioUp = obj["gpioUp"];
      if (obj.containsKey("gpioDown"))
        this->gpioDown = obj["gpioDown"];
      pinMode(this->gpioUp, OUTPUT);
      pinMode(this->gpioDown, OUTPUT);
    }
    if (this->proto == radio_proto::GP_Remote)
    {
      if (obj.containsKey("gpioMy"))
        this->gpioMy = obj["gpioMy"];
...
*/
void SomfyRemote::setRemoteAddress(uint32_t address)
{
  this->m_remoteAddress = address;
  snprintf(this->m_remotePrefId, sizeof(this->m_remotePrefId), "_%lu", (unsigned long)this->m_remoteAddress);
}
...
void SomfyShadeController::processWaitingFrame()
{
  for (uint8_t i = 0; i < SOMFY_MAX_SHADES; i++)
    if (this->shades[i].getShadeId() != 255)
      this->shades[i].processWaitingFrame();
}
...
int8_t SomfyShadeController::getMaxShadeOrder()
{
  ...
  for (uint8_t i = 0; i < SOMFY_MAX_SHADES; i++)
  {
    ...
    if (shade->getShadeId() == 255)
      continue;
    if (order < shade->sortOrder)
      order = shade->sortOrder;
  }
  return order;
}
...
int8_t SomfyShadeController::getMaxRoomOrder()
{
  ...
  for (uint8_t i = 0; i < SOMFY_MAX_ROOMS; i++)
  {
    ...
    if (room->roomId == 0)
      continue;
    if (order < room->sortOrder)
      order = room->sortOrder;
  }
  return order;
}
...
void SomfyRemote::sendCommand(somfy_commands cmd, uint8_t repeat, uint8_t stepSize)
{
  ...
  if (this->lastFrame.bitLength == 0)
    this->lastFrame.bitLength = bit_length;
  if (this->lastFrame.rollingCode == 0)
    Serial.println("ERROR: Setting rcode to 0");
...
void SomfyRemote::repeatFrame(uint8_t repeat)
{
  ...
  {
    this->lastFrame.repeats++;
    if (this->lastFrame.bitLength == 80)
      this->lastFrame.encode80BitFrame(&frm[0], this->lastFrame.repeats);
...
void SomfyShadeController::sendFrame(somfy_frame_t &frame, uint8_t repeat)
{
  ...
  for (uint8_t i = 0; i < repeat; i++)
  {
    ...
    if (frame.bitLength == 80)
      frame.encode80BitFrame(&frm[0], i + 1);
...
void SomfyShadeController::toJSONRepeaters(JsonResponse &json)
{
  for (uint8_t i = 0; i < SOMFY_MAX_REPEATERS; i++)
  {
    if (somfy.repeaters[i] != 0)
      json.addElem((uint32_t)somfy.repeaters[i]);
  }
}
...
void Transceiver::sendFrame(byte *frame, uint8_t sync, uint8_t bitLength)
{
  ...
  if (sync == 2 || sync == 12)
  { // Only with the first frame.  Repeats do not get a wakeup pulse.
...
void RECEIVE_ATTR Transceiver::handleReceive()
{
  ...
  case waiting_synchro:
    ...
    else if (duration > tempo_synchro_sw_min && duration < tempo_synchro_sw_max && somfy_rx.cpt_synchro_hw >= 4)
    {
      ...
      if (somfy_rx.cpt_synchro_hw <= 7)
        somfy_rx.bit_length = 56;
      else if (somfy_rx.cpt_synchro_hw == 14)
        somfy_rx.bit_length = 56;
      else if (somfy_rx.cpt_synchro_hw == 13)
        somfy_rx.bit_length = 80; // The RS485 device sends this sync.
      else if (somfy_rx.cpt_synchro_hw == 12)
        somfy_rx.bit_length = 80;
      else if (somfy_rx.cpt_synchro_hw > 17)
        somfy_rx.bit_length = 80;
      else
        somfy_rx.bit_length = 56;
      // somfy_rx.bit_length = 80;
  ...
  if (somfy_rx.status == receiving_data && somfy_rx.cpt_bits >= somfy_rx.bit_length)
  {
    ...
    if (rx_queue.length >= MAX_RX_BUFFER)
    {
      ...
      if (ndx < MAX_RX_BUFFER)
        rx_queue.items[ndx].pulseCount = 0;
...
void transceiver_config_t::fromJSON(JsonObject &obj)
{
  ...
  if (obj.containsKey("type"))
    this->type = obj["type"];
  if (obj.containsKey("CSNPin"))
    this->CSNPin = obj["CSNPin"];
  if (obj.containsKey("MISOPin"))
    this->MISOPin = obj["MISOPin"];
  if (obj.containsKey("MOSIPin"))
    this->MOSIPin = obj["MOSIPin"];
  if (obj.containsKey("RXPin"))
    this->RXPin = obj["RXPin"];
  if (obj.containsKey("SCKPin"))
    this->SCKPin = obj["SCKPin"];
  if (obj.containsKey("TXPin"))
    this->TXPin = obj["TXPin"];
  if (obj.containsKey("rxBandwidth"))
    this->rxBandwidth = obj["rxBandwidth"]; // float
  if (obj.containsKey("frequency"))
    this->frequency = obj["frequency"]; // float
  if (obj.containsKey("deviation"))
    this->deviation = obj["deviation"]; // float
